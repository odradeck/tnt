        ...
        if auth
          ...
          if email != ''
            ...
            if existinguser
              ...
              user = User.new :email => email, :password => SecureRandom.hex(10), :name => name, :uid=>uid
            ...
            #there is no email like twitter
            existinguser = User.find_by_uid(uid)
            if existinguser
              # map this new login method via a service provider to an existing account if the email address is the same
              existinguser.services.create(:provider => provider, :uid => uid, :uname => name)
              flash[:notice] = 'Sign in via ' + provider.capitalize + ' has been added to your account ' + existinguser.name + '. Signed in successfully!'
              sign_in_and_redirect(:user, existinguser)
            else
              # let's create a new user: register this user and add this authentication method for this user
              name = name[0, 39] if name.length > 39             # otherwise our user validation will hit us
              ...
              # new user, set email, a random password and take the name from the authentication service
              user = User.new :password => SecureRandom.hex(10), :name => name, :uid=>uid
            ...
            # remove for twitter by odradeck 2012.11.2
